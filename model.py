...
# Define the structured response expected from each model
class AIResponse(BaseModel):
    ...
    next_step: str = Field(
        description="Recommended next step for the support representative"
    )
...
# Initialize a watsonx.ai chat model
...
# Initialize the three foundation models
...
def llama_response(system_prompt, user_prompt):
    return get_ai_response(
        llama_llm, llama_template, system_prompt, user_prompt
    )


def granite_response(system_prompt, user_prompt):
    return get_ai_response(
        granite_llm, granite_template, system_prompt, user_prompt
    )


def mistral_response(system_prompt, user_prompt):
    return get_ai_response(
        mistral_llm, mistral_template, system_prompt, user_prompt
    )
